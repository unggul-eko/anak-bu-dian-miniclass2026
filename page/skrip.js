...
setInterval(function checkAlarmTicker() {
  ...
  tasks.forEach((task) => {
    if (
      ...
      document
        .getElementById("liveAlarmAlertBanner")
        .classList.remove("d-none");
      document.getElementById("alarmAlertMessage").innerText =
        `[ALARM KULIAH] ${task.subject.toUpperCase()} - ${task.text}`;
      document
...
function renderCalendar(year, month) {
  document.getElementById("calendarMonthYearTitle").innerText =
  ...
  for (let day = 1; day <= numDays; day++) {
    ...
    if (tasks.some((t) => t.date === loopDateStr && !t.completed)) {
...
function renderTasks() {
  ...
  document
    .querySelectorAll('.todo-item-custom input[type="checkbox"]')
    .forEach((cb) => {
...
function updateStatsUI() {
  ...
  document.getElementById("dashActiveBreakCount").innerText =
    stats.activeBreakCompleted;
  document.getElementById("targetMenitFokus").innerText =
    stats.focusMinutesTotal;
  document.getElementById("totalMenitFokusKeseluruhan").innerText =
    stats.focusMinutesTotal;

  document.getElementById("dashWeeklyStreak").innerText = stats.weeklyStreak;
  document.getElementById("dashTotalSessionsToday").innerText =
    stats.totalFocusSessionsToday;

  document.getElementById("targetTugasSelesai").innerText =
    todayTasksCompletedCount;
  document.getElementById("targetTugasHariIniTotal").innerText =
    targetTugasHariIni;

  document.getElementById("totalTugasSelesaiKeseluruhan").innerText =
    totalSeluruhTugasSelesai;
  document.getElementById("totalTugasKeseluruhanSemua").innerText =
    totalAkumulatifTargetSemua;

  let maxMenit = stats.customMaxMenit || 0;
  let percentToday = 0;
  if (maxMenit > 0) {
    percentToday = Math.min(
      100,
      Math.round((stats.focusMinutesTotal / maxMenit) * 100),
    );
  }
  document.getElementById("dashProgressPercentText").innerText =
    percentToday + "%";
  let ringDashOffsetToday = 408.4 - (408.4 * percentToday) / 100;
  document.getElementById("dashCircularProgressRing").style.strokeDashoffset =
    ringDashOffsetToday;

  let percentAll = 0;
  if (totalAkumulatifTargetSemua > 0) {
    percentAll = Math.min(
      100,
      Math.round((totalSeluruhTugasSelesai / totalAkumulatifTargetSemua) * 100),
    );
  }
  document.getElementById("dashProgressPercentTextAll").innerText =
    percentAll + "%";
  let ringDashOffsetAll = 408.4 - (408.4 * percentAll) / 100;
  document.getElementById(
    "dashCircularProgressRingAll",
  ).style.strokeDashoffset = ringDashOffsetAll;

  document.getElementById("capaianTotalSesi").innerText =
    stats.totalFocusSessionsToday;
  document.getElementById("capaianRataRata").innerText =
...
function renderMemos() {
  ...
  stats.memos.forEach((memo) => {
    if (!groupedMemos[memo.dateLabel]) {
      groupedMemos[memo.dateLabel] = [];
    }
  ...
  for (const dateLabel in groupedMemos) {
    ...
    groupedMemos[dateLabel].forEach((m) => {
...
function updateTimerDisplay() {
  ...
  document.getElementById("timerRingBar").style.width =
    (totalDurationInMode > 0
      ? (currentSeconds / totalDurationInMode) * 100
      : 0) + "%";
}
function stopTimer() {
  if (timerInterval) {
...
function setMode(mode) {
  currentMode = mode;
  document.getElementById("timerModeText").innerText =
    mode === "focus" ? "Fokus Dimulai" : "Waktunya Istirahat";
  document.getElementById("timerModeText").className =
    mode === "focus"
...
const navCapsules = document.querySelectorAll(
...
navCapsules.forEach((pill) => {
  pill.addEventListener("click", function () {
    ...
    if (!targetId) return;
    navCapsules.forEach((p) => p.classList.remove("active"));
...
window.addEventListener("scroll", () => {
  ...
  capsules.forEach((pill) => {
    ...
    if (targetAttr === currentActiveId) {
      pill.classList.add("active");
      const container = document.getElementById("navCapsulesGroup");
      container.scrollTo({
        left:
...
// Inisialisasi Picker Kustom saat modal dibuka
function initCustomPickers() {
  ...
  const dateArea = document.getElementById("datePickerArea");
  const timeArea = document.getElementById("timePickerArea");

  // Toggle Kalender
  dateInput.onclick = () => {
    dateArea.classList.toggle("d-none");
    renderModalCalendar(2026, 5); // Juni 2026
  };

  // Toggle Jam (Wheel)
  timeInput.onclick = () => {
    timeArea.classList.toggle("d-none");
  };
}

function renderModalCalendar(year, month) {
  const container = document.getElementById("calendarDaysContainer");
  container.className = "calendar-grid-modal";
  container.innerHTML = "";
  ...
  for (let d = 1; d <= days; d++) {
    ...
    div.innerText = d;
    div.onclick = () => {
      document.getElementById("formTaskDate").value =
...
// Logika Jam (Scroll)
const jamScroll = document.getElementById("pickJam");
const menitScroll = document.getElementById("pickMenit");
// Isi 00-23 dan 00-59
for (let i = 0; i < 24; i++)
  jamScroll.innerHTML += `<option value="${i}">${i.toString().padStart(2, "0")}</option>`;
for (let i = 0; i < 60; i++)
  menitScroll.innerHTML += `<option value="${i}">${i.toString().padStart(2, "0")}</option>`;

function updateTimeValue() {
  document.getElementById("formTaskTime").value =
    `${jamScroll.value.padStart(2, "0")}:${menitScroll.value.padStart(2, "0")}`;
}
jamScroll.onchange = updateTimeValue;
menitScroll.onchange = updateTimeValue;
