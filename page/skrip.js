...
let stats = { activeBreakCompleted: 0, focusMinutesTotal: 0, weeklyStreak: 0, lastStreakDate: null, totalFocusSessionsToday: 0, memos: [] , customMaxMenit: 0, customMaxTugas: 0 };
let tasks = [];
...
function calculateTotalSecondsFromWheels() {
    ...
    currentSeconds = total; totalDurationInMode = total; 
    updateTimerDisplay();
}
...
function renderTasks() {
    ...
    document.querySelectorAll('.todo-item-custom input[type="checkbox"]').forEach(cb => {
        cb.addEventListener('change', (e) => {
            tasks[parseInt(e.target.getAttribute('data-index'))].completed = e.target.checked; saveData();
        });
    });
    document.querySelectorAll('.btn-delete-task').forEach(btn => {
        btn.addEventListener('click', (e) => {
            tasks.splice(parseInt(e.target.getAttribute('data-index')), 1); saveData();
        });
    });
}
...
function updateStatsUI() {
    ...
    let percentToday = 0;
    if (maxMenit > 0) {
        percentToday = Math.min(100, Math.round((stats.focusMinutesTotal / maxMenit) * 100));
    }
    ...
    if (totalAkumulatifTargetSemua > 0) {
        percentAll = Math.min(100, Math.round((totalSeluruhTugasSelesai / totalAkumulatifTargetSemua) * 100));
    }
...
function startTimer() {
    ...
    if (activeHour && activeMin && activeSec) {
        totalRoda = (parseInt(activeHour.getAttribute('data-val')) * 3600) + 
    ...
    if (!isPaused) {
        if (totalRoda > 0) {
            ...
            if (currentMode === "focus") userFocusSeconds = totalRoda; else userBreakSeconds = totalRoda;
...
document.getElementById('resetDataBtn').addEventListener('click', () => {
    if(confirm("Apakah Anda yakin ingin reset semua data?")) {
        localStorage.clear(); stats = { activeBreakCompleted: 0, focusMinutesTotal: 0, weeklyStreak: 0, lastStreakDate: null, totalFocusSessionsToday: 0, memos: [], customMaxMenit: 0, customMaxTugas: 0 };
...
function renderModalCalendar(year, month) {
    ...
    for(let d=1; d<=days; d++) {
        ...
        div.innerText = d;
        div.onclick = () => {
            document.getElementById('formTaskDate').value = `${year}-${(month+1).toString().padStart(2,'0')}-${d.toString().padStart(2,'0')}`;
...
// Isi 00-23 dan 00-59
for(let i=0; i<24; i++) jamScroll.innerHTML += `<option value="${i}">${i.toString().padStart(2,'0')}</option>`;
for(let i=0; i<60; i++) menitScroll.innerHTML += `<option value="${i}">${i.toString().padStart(2,'0')}</option>`;

function updateTimeValue() {
    document.getElementById('formTaskTime').value = `${jamScroll.value.padStart(2,'0')}:${menitScroll.value.padStart(2,'0')}`;
}
